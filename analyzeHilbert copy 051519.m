...
%% 
rStr='r012'; order={'40a','40s','10s','10a'};
rStr='r013'; order={'10a','10s','40a','40s'};
rStr='r014'; order={'40s','40a','10s','10a'};
rStr='r015'; order={'10s','10a','40a','40s'};
rStr='r016'; order={'40s','40a','10a','10s'};
%rStr='r017'; order={'40s','40a','10a','10s'};
...
yl=[0 200; 0 300]; % first row 40 Hz, second row 10 Hz
...
for i=1:4
    ...
    switch cstr
        case '40a'
            ...
            xlabel('Time (min.)');
            title('Active 40 Hz TUS'); ylim(yl(1,:));
            xlim([-1 4]);
            
            dataCat=mean(varsActiveStim40.env10ds(caChan,:),1);
            subplot(2,4,i+4); hold on
            plot(timeCat,dataCat);
            ylabel('10 Hz envelope');
            xlabel('Time (min.)'); ylim(yl(2,:));
            xlim([-1 4]);
        case '40s'
            ...
            plot(timeCat,dataCat);
            xlabel('Time (min.)');
            title('Sham 40 Hz TUS'); ylim(yl(1,:));xlim([-1 4]);
            
            dataCat=mean(varsShamStim40.env10ds(caChan,:),1);
            subplot(2,4,i+4); hold on
            plot(timeCat,dataCat);
            xlabel('Time (min.)'); ylim(yl(2,:));xlim([-1 4]);
        case '10a'
            dataCat=mean(varsActiveStim10.env40ds(caChan,:),1);
            subplot(2,4,i); hold on
            plot(timeCat,dataCat);
            ylabel('40 Hz envelope');
            xlabel('Time (min.)');
            title('Active 10 Hz TUS'); ylim(yl(1,:));xlim([-1 4]);
        ...
        case '10s'
            dataCat=mean(varsShamStim10.env40ds(caChan,:),1);
            subplot(2,4,i); hold on
            plot(timeCat,dataCat);
            xlabel('Time (min.)');
            title('Sham 10 Hz TUS'); ylim(yl(1,:));xlim([-1 4]);
