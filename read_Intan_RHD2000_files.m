function read_Intan_RHD2000_files(pathToData)
% grab all rhd files in a directory, and save each of their contents to a
% mat file housed in the same directory
...
files=dir(fullfile(pathToData,'*.rhd'));
nFiles=numel(files);
...
%     end
    ...
    tic;
    %filename = [path,file];
    filename = fullfile(pathToData,files(f).name);
    file=files(f).name;
    path=pathToData;
    fprintf(['Processing ' filename '.../n']); 
    ...
    % LoqD, 12.06.18
    matfile=file;
    matfile(end-2:end)='mat';
    save('-v7.3',fullfile(path,matfile),'amplifier_data','amplifier_channels');
