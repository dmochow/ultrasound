...
for f=1:nFilenames-1 % last one is the stimulation
    f
    load(matFilenames{f});
...
load(matFilenames{nFilenames});
stimData=amplifier_data(:,samples{nFilenames}); clear amplifier_data;

%%
% rereference
if ~isnan(rerefChannel)
data = data - repmat (data(rerefChannel,:,:),[size(data,1) 1 1 ]);
stimData = stimData - repmat (stimData(rerefChannel,:,:),[size(stimData,1) 1 1 ]);
end
...
%%
% apply filters
allDataDown40=filter(b40,a40,allDataDown,[],2);
allDataDown10=filter(b10,a10,allDataDown,[],2);
...
save(fullfile(dataFolder,[date '-' saveDataFileStr]),'fsr',...
